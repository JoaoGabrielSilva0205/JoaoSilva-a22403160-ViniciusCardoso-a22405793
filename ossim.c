 ...
 */
void check_new_commands(queue_t *command_queue, queue_t *blocked_queue, queue_t *ready_queue, mlfq_t *mlfq, int server_fd, uint32_t current_time_ms, int scheduler_type) {
    ...
    while (elem != NULL) {
        ...
        if (msg.request == PROCESS_REQUEST_RUN) {
            ...
            // CORREÇÃO: Se for MLFQ (valor 3), adicionar ao MLFQ, senão à ready_queue
            if (scheduler_type == 3) {  // SCHED_MLFQ = 3
                mlfq_enqueue(mlfq, current_pcb, 0); // Nível 0 (maior prioridade)
            } else {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        check_new_commands(&command_queue, &blocked_queue, &ready_queue, &mlfq, server_fd, current_time_ms, (int)scheduler_type);
